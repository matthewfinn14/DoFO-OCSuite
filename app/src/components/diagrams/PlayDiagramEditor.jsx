...
import { MousePointer, Minus, Trash, RotateCcw, RotateCw, Save, X, RefreshCw } from 'lucide-react';
...
const getDefaultWizOLFormation = () => {
  const centerX = 450; // Canvas center X (900/2)
  const centerY = 300; // Canvas center Y (600/2)
...
export default function PlayDiagramEditor({
  ...
  const [endType, setEndType] = useState('t'); // t for T-block by default for OL
    ...
    if (selectedTool === 'line') {
      if (!isDrawing) {
        ...
        const hitPlayer = elements.find(el => {
          ...
          return dist < 60; // generous hit radius for large text
        });
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      {!readOnly && (
        <div className="flex items-center gap-3 p-3 bg-slate-700 border-b border-slate-600 flex-wrap">

          {/* 1. Add Custom Blocker + Text Size */}
          <div className="flex flex-col items-center">
            <span className="text-[10px] text-slate-400 mb-1">Add Blocker</span>
          ...
          {/* 2. Select Button */}
          <div className="flex flex-col items-center">
            <span className="text-[10px] text-slate-400 mb-1">Tool</span>
            <button
          ...
          {/* 3. Line Style + End Type (combined amber box) */}
          <div className="flex flex-col items-center">
            <span className="text-[10px] text-slate-400 mb-1">Line Drawing</span>
            <div className="flex items-center gap-1 bg-amber-900/30 px-2 py-1 rounded border border-amber-700">
              ...
              <div className="w-px h-4 bg-amber-700 mx-1" />
          ...
          {/* 4. Colors */}
          <div className="flex flex-col items-center">
            <span className="text-[10px] text-slate-400 mb-1">Line Color</span>
          ...
          {/* 5. Flip */}
          <div className="flex flex-col items-center">
            <span className="text-[10px] text-slate-400 mb-1">Mirror</span>
            <button
          ...
          {/* 6. Undo/Redo/Delete */}
          <div className="flex flex-col items-center">
            <span className="text-[10px] text-slate-400 mb-1">History</span>
            <div className="flex items-center gap-1">
              <button
                onClick={undo}
          ...
          {/* 7. Cancel, Save As & Save (far right) */}
          <div className="flex flex-col items-end ml-auto">
            <span className="text-[10px] text-slate-400 mb-1">Actions</span>
      ...
      {/* Canvas */}
      <div className="flex-1 bg-slate-900 overflow-hidden flex items-center justify-center p-6">
        <div className="bg-white shadow-xl h-full" style={{ aspectRatio: '900 / 600', maxHeight: '100%' }}>
          <svg
            ref={svgRef}
            width="100%"
            height="100%"
            ...
            style={{ cursor: selectedTool === 'delete' ? 'not-allowed' : (selectedTool === 'line' ? 'crosshair' : 'default') }}
          >
            <defs>
              {COLORS.map(c => (
